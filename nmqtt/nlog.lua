...
function method:send_sys(s)
	print("send sys log:",self.cli:sendto(s, host, server_port))
end

function method:send_real(s)
	self.real_cli:sendto(host, real_port, s)
end

function method:real_start()
	if not self.real_cli then
		self.real_cli = socket.udp()
	end
end
...
local function logfmt(level, fmt, ...)
	...
	func = function()
		...
		print( fmt, level, mod, t.month, t.day, t.hour, t.min, t.sec, src, info.currentline, unpack(vars))
...
-- log.real_enable() and log.real1("%s", js.encode(map))
local function real_enable()
	return next(levels)
end
