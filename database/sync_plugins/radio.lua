...
local function sync(conn_map, tbname, actions, key)
	...
	if #arr > 0 then
		...
		for _, r in ipairs(arr) do
			local devid, band = r.val:match("(.+)_([25]g)")	assert(devid and band)
			table.insert(narr, string.format("(devid='%s' and band='%s')", devid, band))
		end
		...
		while narr and #narr > 0 do
			...
			local sql = string.format("delete from radio where %s", table.concat(del, " or "))
