...
local fs = require("nfs")
local mgr = require("mgr")
local js = require("cjson.safe")

local sync_tables, sync_plugins
local DEFUALT_SYNC_PLUGIN = "###default###"
...
local function load_sync_plugins()
	sync_plugins = {}
	sync_plugins[DEFUALT_SYNC_PLUGIN] = function(conn_map, tbname, actions, key)
		...
		if #arr > 0 then
			...
			local arr = {}
			for k in pairs(map) do table.insert(arr, k) end
			local sql = string.format("select * from %s where %s in (%s)", tbname, key, table.concat(arr, ","))
			print(sql)
	...
	local plugin_dir = "sync_plugins"
	local files = fs.scandir(plugin_dir) or {}
	for _, filename in pairs(files) do
		local tbname = filename:match("(.+)%.lua$")
		if tbname then
			local m = require(string.format("%s.%s", plugin_dir, tbname))
			sync_plugins[tbname] = m.sync
		end
	end
end
...
local function sync_trigger()
	...
	for _, r in ipairs(rs) do
		...
		tbmap[tbname], keymap[tbname] = actions, r.key
	end

	for tbname, actions in pairs(tbmap) do
		local f = sync_plugins[tbname] or sync_plugins[DEFUALT_SYNC_PLUGIN]
		f(ins, tbname, actions, keymap[tbname])
	end
